...
public class RVResultsTable {
	...
	private void printRow(PrintWriter writer, RVResultsTableRow row, boolean correct) throws SpefoException {
		...
		if (!correct) {
			...
			rvCorr = row.getResult("corr").orElseThrow(SpefoException::new) - row.getLstFileRecord().getRvCorr();
		}
		...
		for (String category : categories) {
			...
			nextLine += "\t";
			Optional<Double> rmseOptional = row.getRmse(category);
			if (rmseOptional.isPresent()) {
				nextLine += MathUtils.formatDouble(rmseOptional.get(), 5, 2, false);
			} else {
				nextLine += "        ";
			}
